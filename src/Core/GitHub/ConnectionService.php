...
class ConnectionService
{
    ...
    /**
     * Handles revoked GitHub access gracefully.
     *
     * @param string $appUid The unique identifier for the app.
     * @return array{success:bool,message:string}
     */
    public function handle_revoked_access(string $appUid): array
    {
        try {
            $credentials = $this->credentialService->get_stored_credentials($appUid);
            if (!$credentials) {
                return ['success' => false, 'message' => __('No credentials found for the specified app.', 'wp2-update')];
            }
            ...
            $client->apps()->listRepositories();
            return ['success' => true, 'message' => __('GitHub access is valid.', 'wp2-update')];
        } catch (ExceptionInterface $e) {
            Logger::log('ERROR', 'GitHub access revoked: ' . $e->getMessage());
            $this->credentialService->delete_credentials($appUid);
            return ['success' => false, 'message' => __('GitHub access has been revoked. Please reconnect the app.', 'wp2-update')];
        }
    }
