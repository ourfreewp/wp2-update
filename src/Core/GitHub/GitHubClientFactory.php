...
class GitHubClientFactory
{
    ...
    {
        $resolvedUid = $this->resolveAppUid($appUid);
        if (!$resolvedUid) {
            Logger::log('WARNING', 'No GitHub app UID available for installation token request.');
            return null;
        }

        // Use WordPress transients to cache the token across requests
        $transientKey = 'github_installation_token_' . $resolvedUid;
        ...
        if (!$this->credentialService) {
            Logger::log('ERROR', 'Credential service is not initialised for GitHubClientFactory.');
            return null;
        }

        $credentials = $this->credentialService->get_stored_credentials($resolvedUid);
    ...
    /**
     * Fetches the GitHub API rate limit status.
     *
     * @return array|null The rate limit data, or null on failure.
     */
    public function getRateLimit(): ?array
    {
        $client = $this->getInstallationClient();
        if (!$client) {
            Logger::log('ERROR', 'Failed to fetch rate limit: No authenticated client available.');
            return null;
        }

        try {
            return $client->rateLimit()->getResources();
        } catch (ExceptionInterface $e) {
            Logger::log('ERROR', 'Failed to fetch rate limit: ' . $e->getMessage());
            return null;
        }
    ...
    /**
     * Determine which app UID should be used to generate a token.
     */
    private function resolveAppUid(?string $appUid): ?string
    {
        $appUid = is_string($appUid) ? trim($appUid) : '';
        if ('' !== $appUid) {
            return $appUid;
        }
        ...
        try {
            $credentials = $this->credentialService->get_stored_credentials(null);
        } catch (\Throwable $exception) {
            Logger::log('ERROR', 'Failed to resolve default GitHub app: ' . $exception->getMessage());
            return null;
        }

        $resolvedId = isset($credentials['id']) ? (string) $credentials['id'] : '';
        if ($resolvedId === '') {
            Logger::log('WARNING', 'No GitHub app UID available for installation token request.');
            return null;
        }
        ...
        if (
            ...
            Logger::log('WARNING', sprintf('App %s is missing required credentials for installation tokens.', $resolvedId));
            return null;
        }
