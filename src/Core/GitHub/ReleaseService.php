...
class ReleaseService
{
    ...
    public function download_package(string $url, string $token): ?string
    {
        ...
        $response = wp_remote_get(
            ...
            [
                ...
                'stream'  => true,
                'filename' => $tempFile,
                'redirection' => 0, // Prevent automatic redirects
            ]
        );
        ...
        if ($statusCode === 302 || $statusCode === 301) {
            $redirectUrl = wp_remote_retrieve_header($response, 'location');
            if ($redirectUrl) {
                return $this->download_package($redirectUrl, $token); // Retry with the redirect URL
            }
        }
    ...
    /**
     * Fetches releases for a repository filtered by a specific channel.
     *
     * @param string $owner The repository owner.
     * @param string $repo The repository name.
     * @param string $channel The release channel (e.g., stable, beta, nightly).
     * @return array The filtered releases.
     */
    public function get_releases_by_channel(string $owner, string $repo, string $channel): array
    {
        try {
            $token = $this->clientFactory->getInstallationToken();
            if (!$token) {
            ...
            return array_filter($releases, function ($release) use ($channel) {
                return strpos($release['tag_name'], $channel) !== false;
            });
        } catch (\Exception $e) {
            Logger::log('ERROR', 'Error fetching releases by channel: ' . $e->getMessage());
            return [];
        }
    }
}
